...
export function SearchPage({ onNavigate }) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (active) setStateVersion((value) => value + 1);
    });
    return () => { active = false; };
  }, []);
  ...
  }, [rows, query, category, stateVersion]);
...
export function AccountPage({ session, profile, onProfileUpdate, onNavigate }) {
  ...
  const [dark, setDark] = useState(() => Boolean(readLocalState("kmu-dark-mode", false)));
  ...
  useEffect(() => {
    let active = true;
    Promise.all([
      loadUserState("kmu-interests", []),
      loadUserState("kmu-dark-mode", false),
    ]).then(([nextInterests, nextDark]) => {
      if (!active) return;
      if (Array.isArray(nextInterests)) setInterests(nextInterests);
      setDark(Boolean(nextDark));
      setStateReady(true);
    });
    return () => { active = false; };
  }, []);
  useEffect(() => {
    ...
    if (stateReady) saveUserState("kmu-dark-mode", dark);
  }, [dark, stateReady]);
  ...
  function addInterest(e) {
    ...
    const next = [...interests, value]; setInterests(next); saveUserState("kmu-interests", next); setInterestInput("");
  }
  function clearAiData() {
    if (!window.confirm("AI 일정과 공부 계획 데이터를 모두 삭제할까요?")) return;
    ["kmu-ai-tasks", "kmu-ai-schedule", "kmu-study-plan"].forEach((key) => removeUserState(key));
