...
export default function NotificationPage() {
  ...
  const [aiSchedule, setAiSchedule] = useState(() => {
    const value = readJson("kmu-ai-schedule", []);
    return Array.isArray(value) ? value : [];
  });
  const [studyPlan, setStudyPlan] = useState(() => {
    const value = readJson("kmu-study-plan", []);
    return Array.isArray(value) ? value : [];
  });
  const [tasks, setTasks] = useState(() => {
    const value = readJson("kmu-ai-tasks", []);
    return Array.isArray(value) ? value : [];
  });
  ...
  useEffect(() => {
    let active = true;
    loadUserState("kmu-notification-settings", {}).then((value) => {
      if (!active) return;
      setSettings({ ...defaultSettings, ...(value && typeof value === "object" ? value : {}) });
  ...
  useEffect(() => {
    ...
    ]).then(([schedule, study, taskItems]) => {
      if (!active) return;
      setAiSchedule(Array.isArray(schedule) ? schedule : []);
      setStudyPlan(Array.isArray(study) ? study : []);
  ...
  const update = (key, value) => {
    ...
    setSettings(next);
    if (settingsReady) saveUserState("kmu-notification-settings", next);
  };
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      if (due.length) writeLocalState("kmu-sent-notifications", [...sent, ...due.map((item) => item.id)].slice(-100));
    };
    check();
